...
## Topic-wise quick notes (same format for all major topics)

## 1. Angular Application Bootstrapping

**What is it?**  
Bootstrapping is the startup process where Angular loads the first root component and global providers.
...
**What is it?**  
This is the Angular CLI workflow to scaffold a fresh Angular app.

**How to create:**

```bash
npm install -g @angular/cli
ng new TalentSphereApp
```
...
**What is it?**  
Routing maps URLs to components. This project uses role-based nested routing.

**How it works:**
- Public route: `/login`
- Protected shell route: `/` with `MainLayoutComponent`
- Child routes per role (Admin, Planner, Logistics, Executive, Procurement, Warehouse)
- Fallback route to `/404`

```ts
// src/app/app.routes.ts (pattern)
{
  path: '',
  ...
  children: [
    {
      ...
      data: { roles: ['Planner'] },
      loadComponent: () => import('...').then(m => m.PlannerDashboardComponent)
    }
  ]
}
```
...
**What is it?**  
Authentication validates users and stores login state. Session state is centralized in `AuthenticationService`.

**How it works:**
- Login API returns JWT token
- Frontend decodes token to extract role/user claims
- Stores `authToken` and `currentUser` in localStorage
- Maintains reactive user state with `BehaviorSubject`

```ts
// src/app/services/authentication.service.ts (concept)
...
## 5. Guards and Access Control

**What is it?**  
Guards protect routes before components load.

**How it works in this app:**
- `authGuard`: blocks unauthenticated users
- `RoleGuard`: blocks users without required route role
- Redirects unauthorized users to login or role dashboard

```ts
// src/app/guards/auth.guard.ts (concept)
...
## 6. Route Resolvers (Order Context)

**What is it?**  
Resolvers prepare contextual state before route activation.

**How it works:**
- `OrderContextResolver` => default order visibility
- `ProcurementOrderContextResolver` => hide/disable procurement-incompatible order types
- `WarehouseOrderContextResolver` => hide/disable warehouse-incompatible order types
...
**What is it?**  
Interceptors are middleware for outgoing requests and incoming errors.

**How it works:**
- `authInterceptor` adds `Authorization` header for non-login API calls
- `errorInterceptor` maps technical HTTP errors to user-friendly `userMessage`

```ts
// src/app/interceptors/auth.interceptor.ts (concept)
...
## 8. Layout System (Root, Header, Sidebar)

**What is it?**  
Layout components provide common UI frame for authenticated pages.

**How it works:**
- `App` => global toast container + root outlet
- `MainLayoutComponent` => header + sidebar + feature outlet
- `HeaderComponent` => profile + logout + notifications
- `SidebarComponent` => role-based dynamic menu

---

## 9. Toast and Notification Flow

**What is it?**  
Two user feedback channels:
- Toasts: lightweight action/status messages
- Notification center: inbox-like events with unread counts

**How it works:**
- `ToastService` publishes toast events
- `ToastContainerComponent` renders and dismisses
- `NotificationCenterComponent` loads notifications, polls every 30 sec, supports read/delete actions

---

## 10. Component Architecture (Feature + Shared)

**What is it?**  
Components are organized by business role and shared utilities.

**How it works:**
- Feature folders: `admin-dashboard`, `planner-dashboard`, `logistics-dashboard`, `executive-dashboard`, `procurement-dashboard`, `warehouse-dashboard`
- Shared folder: layout, add-order, view-orders, profile, notifications, toast, not-found
- Standalone components are lazy-loaded for many routes
...
**What is it?**  
Services isolate API calls and reusable business logic.

**How it works:**
- Domain services map to backend resources: users, locations, lanes, partners, inventory, items, uoms, orders, shipments, exceptions, resolutions, KPI, notifications
- Components consume services and bind results to view state
- Some services manage UI context/state (`OrderContextService`, `ToastService`)
...
**What is it?**  
Models define TypeScript contracts for request/response payloads.

**How it works:**
- Each domain has DTO files in `src/app/models`
- `index.ts` provides barrel exports
- `enums.ts` centralizes shared enum values
...
**What is it?**  
Business flows combine routing, UI, services, and APIs.

**Key flows covered in this app:**
- Login and role-based redirect
- Add order (procurement/warehouse constraints)
- Dispatch and shipment lifecycle
- Inventory adjust/create
- Exception and resolution management
- Executive KPI trend/risk/report exploration
- Notification read/clear lifecycle

---

## 14. Configuration and Build System

**What is it?**  
Project-level setup for compile, serve, and test.

**How it works:**
- `angular.json` defines build/serve/test targets
- `tsconfig.json` enables strict mode and path aliases
- `app-settings.ts` stores backend endpoint base URL

---

## 15. Codebase Strengths and Improvement Opportunities

**What is it?**  
A quick engineering quality review.

**Strengths:**
- Clear role-based architecture
- Strong separation: components/services/models/guards/interceptors
- Consistent standalone Angular pattern

**Improvements:**
- Add refresh-token flow
- Replace polling with real-time push for notifications
- Normalize status naming across frontend/backend
